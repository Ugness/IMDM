...
class ReflowDataset(torch.utils.data.Dataset):
    ...
    def load_data(self):
        cache_dir = self.config.reflow.save_path
        ...
        self.seed = np.load(os.path.join(
            cache_dir, 'seeds.npy'), allow_pickle=True) if os.path.exists(os.path.join(cache_dir, 'seeds.npy')) else None
    ...
    def __getitem__(self, idx):
        return {
            ...
            'seed': self.seed[idx] if hasattr(self, 'seed') else 0,
        }
...
def get_tokenizer(config):
    if config.data.tokenizer_name_or_path == 'text8':
        ...
        tokenizer = SyntheticTokenizer(vocab_size=config.data.vocab_size+1)
    else:
...
def get_dataloaders(config, tokenizer, skip_train=False,
                                        skip_valid=False, valid_seed=None, force=False):
...
def get_cond_dataloaders(config, tokenizer, num_proc=1, min_seq_len=1024, skip_train=False,
                                        skip_valid=False, valid_seed=None, force=False):
    num_gpus = torch.cuda.device_count()
    if not force:
        assert (config.loader.global_batch_size
    ...
    if skip_valid:
        ...
        valid_set = valid_set.filter(
            lambda example: example['input_ids'].shape[0] >= min_seq_len,
            num_proc=num_proc)
