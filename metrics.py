...
class Perplexity(NLL):
    ...
    def samples_perplexity(self) -> typing.List[float]:
        """Returns the list of perplexities for each sample.

        Returns:
            List of perplexities for each sample.
        """
        result = self.samples_ppl
        self.samples_ppl = []
        return result
    ...
    def update(self,
        ...
        # self.samples_ppl.extend(torch.exp(value.sum(dim=-1) / weight.sum(dim=-1)).cpu().tolist()) # When eval move, it should be commented out
...
class Metrics:
    ...
    def record_entropy(self, tokens):
        ...
        return np.array(entropys).flatten()
        ...
        if 'llama' not in self.gen_ppl_eval_model_name_or_path:
            ...
            with torch.inference_mode():
                ...
                for i in range(num_batches):
                    ...
                    self.gen_ppl.update(nlls * valid_tokens, valid_tokens)
        return np.array(self.gen_ppl.samples_perplexity()).flatten()
